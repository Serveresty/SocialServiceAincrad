...
func GetProfileById(id int) (*models.ProfileData, error) {
	...
	row2 := database.DB.QueryRow(context.Background(), `SELECT i.short_info, i.family_state, i.born_city, i.current_resident, s.personal_web, s.instagram, s.steam FROM info i JOIN users_info ui ON ui.user_id = $1 AND ui.info_id = i.info_id JOIN social s ON i.social_info = s.social_id`, id)
...
func GetProfileByUsername(username string) (*models.ProfileData, error) {
	row1 := database.DB.QueryRow(context.Background(), `SELECT first_name, last_name, sex FROM "users_data" WHERE username = $1`, username)
	var profileData models.ProfileData
	err := row1.Scan(&profileData.First_name, &profileData.Last_name, &profileData.Sex)
	if err != nil {
		return nil, err
	...
	// row3 := database.DB.QueryRow(context.Background(), `SELECT first_name, last_name, sex FROM "users_data" WHERE username = $1`, username)
	// err = row3.Scan()
	// if err != nil {
	// 	return nil, err
	// }
